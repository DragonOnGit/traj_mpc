...
### 修复
- **cmd_vel话题冲突导致X/Y误差和抖动**：
  - **问题**：另一个节点同时发布到 `/robot1/robot/cmd_vel`，导致MPC指令被覆盖，X/Y方向出现0.917m稳态误差和飞行器抖动
  - **修复1**：新增 `cmd_vel_hold_timer_`（默认50Hz），在非控制周期持续重发最后一条指令，压制冲突发布者
  - **修复2**：cmd_vel发布队列从10改为1，确保最新指令立即送达
  - **修复3**：控制频率从10Hz提高到20Hz，减少外部节点覆盖窗口
  - **修复4**：新增 `cmd_vel_hold_rate` 参数（默认50Hz），可配置

### 优化
- **MPC参数优化**：提高X/Y方向跟踪精度
  - `weight_pos_x/y`: 25.0 → 50.0（更激进的位置跟踪）
  - `weight_acc_x/y`: 0.10 → 0.05（允许更激进的加速度）
  - `weight_vel_x/y`: 0.5 → 1.0（增加速度阻尼，减少超调）
  - `control_rate`: 10Hz → 20Hz（更快的控制响应）

### 新增
- **不匹配率统计**：echo检测新增 `mismatch_count_` 和 `total_echo_checks_`，输出不匹配率百分比
- **cmd_vel_hold_rate参数**：launch文件新增指令保持频率配置
